// 코디 추천 조건과 추천 결과 목록을 보여주는 컴포넌트입니다.
...
// 추천 코디 한 장을 카드로 렌더합니다. catalogIds를 넘기면 아이템마다 보유/카탈로그 출처 뱃지를 표시합니다.
  ...
  return (
    <article className="panel outfit-card" key={outfit.id}>
      ...
      <div className="recommend-item-strip">
        {outfit.items.map((item) => (
          <div key={item.id} className="outfit-item-thumb" title={item.fitReason}>
            ...
            {catalogIds && <span className={`source-badge ${catalogIds.has(item.id) ? 'source-catalog' : 'source-owned'}`}>{catalogIds.has(item.id) ? '카탈로그' : '보유'}</span>}
      ...
      <div className="recommendation-reason-panel">
        <p className="reason-headline">{outfit.reason}</p>
