...
// HEX 색상을 COLOR_META 팔레트에서 CIEDE2000 거리가 가장 가까운 색으로 매핑합니다.
// AI가 추출한 임의 HEX를 우리 색 분류 체계(색 이름·중립·데님)에 정합시키는 기준점입니다.
function nearestColorMeta(hex: string) {
  const lab = rgbToLab(hexToRgb(hex));
  let best = COLOR_META.화이트;
  let bestDist = Infinity;
  for (const meta of Object.values(COLOR_META)) {
    const dist = deltaE2000(lab, rgbToLab(hexToRgb(meta.hex)));
    if (dist < bestDist) {
      bestDist = dist;
      best = meta;
    }
...
// 색 입력을 표준 메타(이름·HEX·중립·데님)로 정규화합니다.
// 이름 입력은 COLOR_META를 그대로, HEX 입력은 가장 가까운 색의 이름·플래그를 따르되 HEX 자체는 보존합니다.
export function colorMetaForInput(color: string) {
  if (isHexColor(color)) {
    const nearest = nearestColorMeta(color);
    return { representative: nearest.representative, hex: color.toUpperCase(), neutral: nearest.neutral, denim: nearest.denim };
  }
...
export function buildColorMeta(category: ClothingCategory, type: string, color: string, colors?: ClothingColorAnalysis[], sourceText = '') {
  ...
  const isHexInput = isHexColor(color);
  // 데님 판정. 이름 입력은 기존 동작 그대로(회귀 보존). HEX 입력은 가장 가까운 색이 데님이고 하의·아우터일 때만 인정(상의 오탐 방지).
  const denimByName = Boolean(COLOR_META[color]?.denim);
  const denimByHex = isHexInput && (category === '하의' || category === '아우터') && Boolean(colorMeta.denim);
  const isDenim = material === 'denim' || denimByName || denimByHex || /청|데님|jean|denim/i.test(text);
  ...
  // 중립 판정. 이름 입력은 COLOR_META 그대로, HEX 입력은 가장 가까운 색의 중립 플래그를 따른다.
  const isNeutral = isHexInput ? Boolean(colorMeta.neutral) : Boolean(COLOR_META[color]?.neutral);
  return {
    representativeColor,
