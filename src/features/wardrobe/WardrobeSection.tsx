  ...
  return (
    <section className="wardrobe-page">
      ...
      <div className={props.detailLayout === 'list' ? 'clothing-grid list-view' : 'clothing-grid'}>
        {filtered.map((item) => <ClothingCard key={item.id} item={item} onDelete={() => props.onDeleteItem(item.id)} onFindOutfits={props.onFindOutfits} />)}
      </div>
    </section>
  );
}
...
function ClothingCard({ item, onDelete, onFindOutfits }: { key?: React.Key; item: ScoredClothingItem; onDelete: () => void; onFindOutfits: (item: ScoredClothingItem) => void }) {
  return (
    <article className="clothing-card">
      ...
      <div className="clothing-body">
        ...
        <div className="clothing-card-actions">
          {ANCHORABLE_CATEGORIES.includes(item.category) && (
            <button className="line-button compact" type="button" onClick={() => onFindOutfits(item)}><Sparkles size={13} /> 이 옷으로 코디</button>
          )}
